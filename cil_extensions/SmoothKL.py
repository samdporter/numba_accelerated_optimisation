...
class SmoothApproxKullbackLeibler(Function):
    ...
    *Stochastic Primal-Dual Hybrid Gradient Algorithm with Arbitrary Sampling
    ...
    def __init__(self, **kwargs):
        ...
        self.b = kwargs.get('b', None)
        self.eta = kwargs.get('eta',None)    
        self.approx = kwargs.get('approx',self.eta/2)
        
        self.b_np = self.b.as_array()
        self.eta_np = self.eta.as_array()
        self.approx_np = self.approx.as_array()
        ...
        super(SmoothApproxKullbackLeibler, self).__init__(L = numpy.max(self.b_np / self.eta_np ** 2))
    ...
    def __call__(self, x):
        ...
        return _call_numba(x.as_array(), self.b_np, self.eta_np, self.approx_np)
    ...
    def gradient(self, x, out=None):
        ...
        _gradient_numba(x.as_array(), self.b_np, self.eta_np, self.approx_np, out_arr)
...
@njit(parallel=True)
def _call_numba(x_arr, b_np, eta_np, approx):
    counter = 0

    for i in prange(x_arr.size):
        x = x_arr.flat[i]
        e = eta_np.flat[i]
        b = b_np.flat[i]
        ...
        if x >= approx.flat[i]:
            ...
            if b == 0:
                ...
                a = (b / (2 * e**2) * x**2 + (1 - b / e) * x +
                        e - b + b * numpy.log(b / e))
        counter += a    
    
    return counter

def _gradient_numba(x_arr, b_np, eta_np, approx, out):
    ...
    for i in prange(x_arr.size):
        ...
        if x >= approx.flat[i]:
            ...
            out.flat[i] = b * x / e**2 + 1. - b / e

def _convex_conjugate_numba(x_arr, b_np, eta_np, approx):
    counter = 0
    ...
    for i in prange(x_arr.size):
        ...
        if x >= approx.flat[i]:
            a = 0.5 * (x - e + b + numpy.sqrt((x - e + b)**2 + 4 * b * e))
        else:
            a = 0.5 * (x - e + b + numpy.sqrt((x - e + b)**2 + 4 * b * e))
        counter += a
...
def _proximal_conjugate_numba(x_arr, b_np, eta_np, approx, tau, out):
    ...
    for i in prange(x_arr.size):
        ...
        if x >= approx.flat[i]:
            out.flat[i] = (b * x - tau * e * b + tau * e**2) / (b + tau * e**2)
        else:
            out.flat[i] = x + tau * e + 1. - numpy.sqrt((x + tau * e - 1.)**2 + 4. * tau * b) / 2.
